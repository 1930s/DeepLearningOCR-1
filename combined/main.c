    ...
    if (fontFile == NULL) {
	  ...
	  PyObject *mymodule = NULL;
		...
		if (isDL == true) {
			  ...
			  mymodule = PyImport_ImportModule("dl");
			  if (mymodule == NULL) {
			    	PyErr_Print();
			    	exit(1);
			  }
			  myfunc = PyObject_GetAttrString(mymodule, "load_saved_model");
			  if (myfunc == NULL) {
			    	PyErr_Print();
			    	exit(1);
			  }

			  fontDataName = Py_BuildValue("s", fontFile);
			  // Call the Python function using the arglist and get its result
			  result = PyObject_CallFunctionObjArgs(myfunc,
						fontDataName, NULL);
			  if (result == NULL) {
			    	PyErr_Print();
			    	exit(1);
		...
		// Both SVM and KD don't need to pre-load files like this
